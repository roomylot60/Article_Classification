...
### 요약 모델 튜닝 관련 개선사항
#### 1. 데이터 전처리 및 증강 개선
- 문장 단위로 텍스트를 분리하고 전처리하는 함수 추가
- 문장 순서를 섞는 데이터 증강 기법 도입
- 빈 문장 제거 등 기본적인 텍스트 정제

#### 2. 하이퍼파라미터 최적화
- Optuna를 사용한 자동 하이퍼파라미터 튜닝 도입
- 학습률, 배치 크기, 에포크 수, 가중치 감쇠 등의 파라미터 최적화
- 학습률 스케줄링과 웜업 추가

#### 3. 평가 메트릭 확장
- ROUGE 외에 BLEU, METEOR 점수 추가
- 결과 시각화를 위한 그래프 생성
- 더 포괄적인 모델 성능 비교 가능

#### 4. 생성 파라미터 최적화
- `no_repeat_ngram_size` 추가로 반복되는 n-gram 방지
- temperature 파라미터 조정으로 생성 다양성 제어
- 빔 서치 파라미터 최적화

### 추가로 고려할 수 있는 개선 사항
#### 1. 데이터 품질 개선:
- 더 많은 양의 고품질 학습 데이터 수집
- 데이터 라벨링 품질 검증 및 개선

#### 2. 모델 아키텍처 개선:
- 더 큰 모델 사용 (예: KoBART-large)
- 앙상블 기법 도입
- 트랜스포머 기반의 최신 아키텍처 시도
...
## 자동 크롤링
### 1. 시스템 서비스 파일 생성
- `article_scraper_service.py`, `article-scraper.service`
...
### 서비스를 설치 및 실행하는 방법
1. 서비스 파일을 시스템 디렉토리로 복사:

```bash
sudo cp article-scraper.service /etc/systemd/system/
```
